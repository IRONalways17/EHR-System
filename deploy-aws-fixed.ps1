...
if ($LASTEXITCODE -ne 0) {
    Write-Host "CloudFormation packaging failed" -ForegroundColor Red
    exit 1
}
...
Write-Host "Creating frontend bucket: $FRONTEND_BUCKET" -ForegroundColor Yellow
...
Set-Content -Path bucket-policy.json -Value $policyJson
...
Write-Host "1. Access the frontend at: $FRONTEND_URL" -ForegroundColor White
Write-Host "2. Enable Bedrock model access in AWS Console (Bedrock > Model access)" -ForegroundColor White
Write-Host "3. Test the image enhancement and clinical notes features" -ForegroundColor White
Write-Host ""
